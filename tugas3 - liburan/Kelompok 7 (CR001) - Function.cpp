...
#include "Kelompok 7 (CR001) - Library.hpp"
...
void sequentialSearch(const vector<string>& data) {
	systemClose();
	vector<string> targets;
	string targetInput;
    cetak("Masukkan elemen yang ingin dicari (pisahkan dengan ', ' jika lebih dari satu) : ");
    ...
    for (const auto& target : targets) {
        ...
        for (size_t i = 0; i < data.size(); ++i) {
            if (data[i] == target) {
                cetak("Sequential Search : Elemen '", target, "' ditemukan pada nomor ", to_string(i + 1), " (indeks ", i, ")\n");
                found = true;
                break;
            }
        }
        if (!found) {
            cetak("Sequential Search : Elemen '", target, "' tidak ditemukan.\n");
        }
    }
    cetak("\n\n");
}

void binarySearch(const vector<string>& data) {
	systemClose();
	vector<string> targets;
	string targetInput;
    cetak("Masukkan elemen yang ingin dicari (pisahkan dengan ', ' jika lebih dari satu) : ");
    ...
    for (const auto& target : targets) {
        ...
        while (left <= right) {
            ...
            if (sortedData[mid] == target) {
                int index = find(data.begin(), data.end(), target) - data.begin();
                cetak("Binary Search : Elemen '", target, "' ditemukan pada nomor ", to_string(index + 1), " (indeks ", index, ")\n");
                found = true;
                break;
        ...
        if (!found) {
            cetak("Binary Search : Elemen '", target, "' tidak ditemukan.\n");
        }
    }
    cetak("\n\n");
}
